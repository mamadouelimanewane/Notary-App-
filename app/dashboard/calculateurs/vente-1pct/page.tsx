...
import { calculerVente, formaterMontantVente, VenteInput } from '@/lib/calculators/vente-1pct';

export default function VentePage() {
    const [formData, setFormData] = useState<VenteInput>({
        prix: 50000000,
    ...
    const handleCalculate = () => {
        const calculResult = calculerVente(formData);
        ...
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
                    ...
                    Calcul Frais et Honoraires - Vente Immobilière (1%)
                </h1>
                <p className="text-muted-foreground mt-1">Barème notarial pour vente avec enregistrement à 1%</p>
            </div>
                            ...
                            {result.lignes.map((ligne: any, index: number) => (
                                ...
                                >
                                    ...
                                    <span className={`font-mono ${ligne.isBold ? 'font-bold' : ''} ${ligne.label === 'TOTAL' ? 'text-blue-700 text-lg' : ''
                                        ...
                                        {formaterMontantVente(ligne.montant)}
                                    </span>
                                </div>
                            ))}
                        </div>
            ...
            <div className="rounded-xl border bg-blue-50 p-6">
                <h3 className="font-semibold mb-3">ℹ️ Barème - Vente Immobilière (1%)</h3>
                <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-700">
                    <div>
                        <p className="font-semibold mb-2">Tranches de prix :</p>
                        <ul className="space-y-1">
                            <li>• 0 à 20 millions : <strong>2,25%</strong></li>
                    ...
                    <div>
                        ...
                        <ul className="space-y-1">
                            ...
                            <li>• Divers : <strong>60 000 FCFA</strong></li>
                        </ul>
                    </div>
                </div>
                <div className="mt-4 p-3 bg-white rounded border border-blue-200">
                    <p className="text-sm"><strong>Note :</strong> Ce barème s'applique aux ventes immobilières avec un taux d'enregistrement réduit à 1% (au lieu de 5%). Les honoraires sont calculés selon un barème progressif plus avantageux.</p>
                </div>
            </div>
        </div>
    );
