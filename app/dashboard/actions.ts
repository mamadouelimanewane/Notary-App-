...
export async function createClient(formData: FormData) {
    ...
    // Créer automatiquement le compte client (411.XXXX) dans la comptabilité
    try {
        const { ClientAccountService } = await import("@/lib/accounting/client-account-service");
        const clientName = type === "ENTREPRISE" && companyName
            ? companyName
            : `${firstName} ${lastName}`;
        ClientAccountService.createClientAccount(client.id, clientName);
    } catch (error) {
